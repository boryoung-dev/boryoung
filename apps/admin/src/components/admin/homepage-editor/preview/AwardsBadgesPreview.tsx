"use client";

import type { Curation } from "../HomepageEditor";
import { PreviewShell } from "./_shared/PreviewShell";
import {
  alignClass,
  renderLines,
  themeColors,
  titleSizeClass,
  type PreviewStyleConfig,
} from "./_shared/styleHelpers";

const COL_MAP: Record<3 | 4 | 5 | 6, string> = {
  3: "grid-cols-3",
  4: "grid-cols-4",
  5: "grid-cols-5",
  6: "grid-cols-6",
};

/** 수상 배지 미리보기 */
export function AwardsBadgesPreview({ curation }: { curation: Curation }) {
  const cfg = curation.displayConfig || {};
  const style: PreviewStyleConfig = cfg.style || {};
  const badges = (cfg.badges || []) as Array<{
    imageUrl: string;
    name: string;
    year?: string;
  }>;
  const columns = (cfg.columns || 4) as 3 | 4 | 5 | 6;
  const theme = themeColors(style.textTheme);

  return (
    <PreviewShell style={style}>
      {(curation.title || curation.subtitle) && (
        <div className={`mb-3 ${alignClass(style.textAlign)}`}>
          {curation.subtitle && (
            <p className={`text-[10px] font-medium uppercase tracking-wide ${theme.muted}`}>
              {curation.subtitle}
            </p>
          )}
          {curation.title && (
            <h2 className={`${titleSizeClass(style.fontSize?.title)} font-semibold ${theme.title}`}>
              {renderLines(curation.title)}
            </h2>
          )}
        </div>
      )}
      {badges.length > 0 ? (
        <div className={`grid ${COL_MAP[columns]} gap-2`}>
          {badges.slice(0, 8).map((badge, i) => (
            <div key={i} className="flex flex-col items-center text-center">
              {badge.imageUrl ? (
                <img
                  src={badge.imageUrl}
                  alt={badge.name}
                  referrerPolicy="no-referrer"
                  className="w-8 h-8 object-contain"
                />
              ) : (
                <div className="w-8 h-8 bg-gray-200 rounded" />
              )}
              <p className={`mt-1 text-[10px] font-semibold leading-tight ${theme.title} truncate w-full`}>
                {badge.name}
              </p>
              {badge.year && (
                <p className={`text-[9px] ${theme.muted}`}>{badge.year}</p>
              )}
            </div>
          ))}
        </div>
      ) : (
        <p className={`text-xs text-center ${theme.muted}`}>배지를 추가하세요</p>
      )}
    </PreviewShell>
  );
}
