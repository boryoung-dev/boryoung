"use client";

import { ImageIcon, Star } from "lucide-react";
import { PreviewShell } from "./_shared/PreviewShell";
import {
  themeColors,
  titleSizeClass,
  type PreviewStyleConfig,
} from "./_shared/styleHelpers";

interface Product {
  id?: string;
  title: string;
  imageUrl?: string;
  slug?: string;
  destination?: string;
  duration?: string;
  basePrice?: number;
  originalPrice?: number;
  rating?: number;
  reviewCount?: number;
  badge?: string;
}

interface Curation {
  title: string;
  subtitle?: string | null;
  linkUrl?: string | null;
  displayConfig?: any;
  products?: Product[];
}

/** 상품 캐러셀 (가로 슬라이드) 미리보기 - 정적 flex */
export function ProductCarouselPreview({ curation }: { curation: Curation }) {
  const products = curation.products || [];
  const style: PreviewStyleConfig = curation.displayConfig?.style || {};
  const theme = themeColors(style.textTheme);

  return (
    <PreviewShell style={style}>
      <div className="flex items-center justify-between mb-4">
        <h2
          className={`${titleSizeClass(style.fontSize?.title)} font-bold tracking-tight ${theme.title}`}
        >
          {curation.title}
        </h2>
        <span
          style={{ color: "var(--brand, #0071e3)" }}
          className="text-[11px] font-medium"
        >
          더보기
        </span>
      </div>

      {products.length === 0 ? (
        <div
          style={{
            background: "var(--surface, #f5f5f7)",
            color: "var(--muted, #86868b)",
          }}
          className="flex flex-col items-center justify-center py-16 rounded-xl"
        >
          <ImageIcon className="w-10 h-10 mb-2 opacity-40" />
          <p className="text-sm">연결된 상품이 없습니다</p>
        </div>
      ) : (
        <div className="flex gap-3 overflow-hidden">
          {products.slice(0, 4).map((product, i) => (
            <div
              key={product.id || i}
              className="flex-shrink-0"
              style={{ width: "calc(25% - 9px)" }}
            >
              <div
                className="relative aspect-[4/3] rounded-xl overflow-hidden mb-2"
                style={{ background: "var(--surface, #f5f5f7)" }}
              >
                {product.imageUrl ? (
                  <img
                    src={product.imageUrl}
                    alt={product.title}
                    referrerPolicy="no-referrer"
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <ImageIcon
                      className="w-8 h-8"
                      style={{ color: "var(--muted, #86868b)" }}
                    />
                  </div>
                )}
                {product.badge && (
                  <span className="absolute top-2 left-2 px-1.5 py-0.5 bg-red-500 text-white text-[9px] font-bold rounded">
                    {product.badge}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-1 mb-0.5">
                <span className={`text-[10px] ${theme.muted}`}>
                  {product.destination}
                </span>
                {product.duration && (
                  <>
                    <span style={{ color: "var(--border, #d2d2d7)" }}>·</span>
                    <span className={`text-[10px] ${theme.muted}`}>
                      {product.duration}
                    </span>
                  </>
                )}
              </div>
              <h3
                className={`text-[12px] font-semibold line-clamp-1 mb-1 ${theme.title}`}
              >
                {product.title}
              </h3>
              {(product.rating ?? 0) > 0 && (
                <div className="flex items-center gap-0.5 mb-1">
                  <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
                  <span
                    className={`text-[10px] font-semibold ${theme.title}`}
                  >
                    {product.rating?.toFixed(1)}
                  </span>
                </div>
              )}
              <div className="flex items-baseline gap-1">
                {product.originalPrice &&
                  product.basePrice &&
                  product.originalPrice > product.basePrice && (
                    <span className="text-[10px] font-bold text-red-500">
                      {Math.round(
                        (1 - product.basePrice / product.originalPrice) * 100
                      )}
                      %
                    </span>
                  )}
                {product.basePrice ? (
                  <span
                    className={`text-[13px] font-bold ${theme.title}`}
                  >
                    {product.basePrice.toLocaleString()}원
                  </span>
                ) : (
                  <span className={`text-[11px] ${theme.muted}`}>
                    가격 문의
                  </span>
                )}
              </div>
            </div>
          ))}
        </div>
      )}
    </PreviewShell>
  );
}
