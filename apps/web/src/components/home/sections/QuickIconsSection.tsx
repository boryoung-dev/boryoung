       ...
       <div className="mt-8 px-4 max-w-[1200px] mx-auto">
        <div className="grid grid-cols-4 md:grid-cols-8 gap-y-6 gap-x-2 md:gap-6">
          {props.items.map((item, idx) => (
            ...
            >
              <div className="relative w-14 h-14 md:w-16 md:h-16 rounded-2xl md:rounded-[20px] bg-gray-50 text-gray-600 flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform duration-300">
                {ICONS[item.iconName] || DEFAULT_ICON}
              </div>
              <span className="text-xs md:text-sm font-medium text-gray-700 group-hover:text-black transition-colors">
                {item.label}
              </span>
            </a>
          ))}
