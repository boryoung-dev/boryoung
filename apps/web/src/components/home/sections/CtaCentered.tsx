...
/** 중앙 정렬 CTA 블록 */
export function CtaCentered({ curation }: { curation: CurationSection }) {
  const cfg = (curation.displayConfig || {}) as CtaCenteredConfig;
  const style = {
  ...
  if (!curation.title && !curation.description && buttons.length === 0) {
    return null;
  }
